...
The rule under test is **R5 `shared_chain_corporate_group`** as defined in `src/rules.yml` and re-stated formally in `reports/logic/rule_inference_report.md`:
...
import json, random
random.seed(7)
facts = json.load(open("reports/logic/rule_inference_facts.json"))["facts"]
...
The exact 30 IDs as labelled here are stored in `reports/logic/rule_eval_corporate_sibling.json`.
