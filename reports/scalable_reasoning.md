...
|---|---|---|
| Rule materializer | Applies 4 forward-chaining rules to derive new facts | `src/materialize_rules.py` over the unified CSV |
| SHACL validator | Checks 6 shapes over the RDF graph | `src/validate_graph.py` |
| TransE link scorer | Embedding-based ranking of weak candidate links | `src/score_candidates.py` |
| Inferred-fact RDF emitter | Writes derived edges as a separate Turtle file | `src/materialize_rules.py` (writes `graph/inferred_facts.ttl`) |
