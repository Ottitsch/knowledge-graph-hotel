...
- **Triple it participates in:** the same triple as 1.1 with the operator on the tail side. Its 0.27 distance gap to the corrupted operator is precisely what shows up in `reports/candidate_scores.csv` as the embedding ranking signal.
...
- Embedding-ranked plausibility score (normalised, from `reports/candidate_scores.csv`): **0.9956**, ranking it the second-strongest candidate in the entire weak-candidate set (1 097 rows).
