...
def _rule_breakdown(previous_path: Path, current_path: Path) -> list[str]:
    prev = _load_snapshot_json(previous_path, RULE_SUMMARY_JSON.name)
    cur = _load_snapshot_json(current_path, RULE_SUMMARY_JSON.name)
    prev_by_type = prev.get("facts_by_type", {})
    cur_by_type = cur.get("facts_by_type", {})
    types = sorted(set(prev_by_type) | set(cur_by_type))
    rows = [
        "| Inferred type | Previous | Current | Δ |",
        "|---|---:|---:|---:|",
    ]
    for t in types:
        p = int(prev_by_type.get(t, 0))
        c = int(cur_by_type.get(t, 0))
        delta = c - p
        sign = "+" if delta > 0 else ""
        rows.append(f"| `{t}` | {p} | {c} | {sign}{delta} |")
    return rows


def build_markdown(summary: dict, details: dict, previous_path: Path | None = None, current_path: Path | None = None) -> str:
    lines = [
        "# Evolution Report",
        "",
    ...
    if previous_path is not None and current_path is not None:
        lines.extend([
            "## Rule fact breakdown",
            "",
        ...
        lines.extend(_rule_breakdown(previous_path, current_path))
        lines.append("")
...
def main() -> None:
    ...
    EVOLUTION_REPORT_MD.write_text(
        build_markdown(summary, details, previous_path, current_path), encoding="utf-8"
    )
